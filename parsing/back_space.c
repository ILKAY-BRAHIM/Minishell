...
/*   Updated: 2023/05/11 22:21:14 by bchifour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_table *back_space(t_table *table)
{
	...
	i = -1;
	table->commend= remove_char(table->commend, '\6');
	table->commend = hanty_7ada_wlat_wa7da_khera(table->commend);
	while(table->arg[++i])
	{
		table->arg[i]= remove_char(table->arg[i], '\6');
	...
	{
		table->option[i]= remove_char(table->option[i], '\6');
